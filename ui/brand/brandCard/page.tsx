...
export default function BrandCard({ brand, index, onDeleteClick, onEditClick }: BrandCardProps) {
    ...
    return (
        <div
            className='flex items-center flex-row bg-white hover:bg-zinc-300 cursor-pointer border-b-2 p-4 mb-4 transition duration-300 ease-in-out'
            onClick={() => { setOpen(true) }}>
            <div className='flex-1 text-gray-600 mr-4'>{index}</div>
            <div className='flex-1 mr-4'>
                <img src={brand.image} alt='logo' className='rounded-md w-10 h-10' />
            </div>
