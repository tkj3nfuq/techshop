...
export default function BrandEditTable({ brandID }: UserEditTableProps) {
    ...
    return (
        <div className='flex flex-col mx-10 mt-2 mb-8 px-6 py-4 rounded-xl shadow-md bg-white'>
            ...
            <Input name="Email" value={updatedBrand.email || ""} onChange={(value) => setUpdatedBrand({ ...updatedBrand, email: value })} />
            <Input name="Phone Number" value={updatedBrand.phoneNumber || ""} onChange={(value) => setUpdatedBrand({ ...updatedBrand, phoneNumber: value })} />
            <MultilineInput name="Description" value={updatedBrand.description || ""} onChange={(value) => setUpdatedBrand({ ...updatedBrand, description: value })} />
            <button
